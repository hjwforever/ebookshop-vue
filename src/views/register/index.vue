...
export default {
  ...
  data() {
    ...
    const validatePassword = (rule, value, callback) => {
      if (value.length < 6) {
        callback(new Error("The password can't less than 6 digits"))
      } else if (value.length > 16) {
        callback(new Error("The password can't more than 16 digits"))
      } else {
        callback()
      }
    }
    ...
    return {
      ...
      registerRules: {
        ...
        email: [
          { required: true, message: 'Please enter current emil', trigger: 'blur' },
          { type: 'email', message: 'Please enter the correct email format, such "zhangsan@ebookshop.com"', trigger: ['blur', 'change'] }
        ],
        password: [
